...
root = 'subjects'
tract_names = pd.read_csv('tract_name_Lan.txt', header=None)[0] 

for subject_path in glob.glob(root+'/*'):
	for tract_name in tract_names:
		bundle_path = (subject_path + '/tractseg_output/bundle_segmentations/' +
					   tract_name + '.nii.gz')
		bundle_b_path = (subject_path + '/tractseg_output/endings_segmentations/' +
						 tract_name + '_b.nii.gz')
		bundle_e_path = (subject_path + '/tractseg_output/endings_segmentations/' +
						 tract_name + '_e.nii.gz')

		os.system('tckgen -algorithm iFOD2 '+subject_path+ '/5_DWI/wm.mif '
				  +subject_path+'/tractseg_output/FOD_iFOD2_trackings/'
		...
		os.system('tckresample '+subject_path+'/tractseg_output/FOD_iFOD2_trackings/'
				  +tract_name+'.tck -num_points 100 '
				  +subject_path+'/tractseg_output/FOD_iFOD2_trackings/'
				  +tract_name+'_N100.tck')
